...
校对页默认忠实使用 MinerU 原文，通用续接检测只写入候选数据。用户明确要求通用自动续接时，可加 `--auto-continuations`；这是基于版面、英文边界、符号和跨页提示的规则合并，仍需核对原图。只有用户要求落实已复核的修复时，才使用显式的 `--profile-dir`；匹配源哈希的清单优先于通用规则。本 Skill 不携带或自动加载旧文档 profile。
...
脚本生成完整的编号原文、PDF 页码/块映射、覆盖清单及 `continuation-candidates.json`，**不生成语义判断**。候选包含断点字串、行号、页码和被跨过的图表块；`autoEligible` 只表示通用规则是否可选用，不表示已经核对。Codex 必须阅读所有主检查分块，结合前后段、标题、双栏和跨页位置，对疑点查看原始页图后，亲自写出 `<结果目录>/修复建议.md`。
